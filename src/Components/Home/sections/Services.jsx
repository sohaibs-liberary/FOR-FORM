import React from 'react'
import Easyexchange from "/src/assets/images/download (1).png"
import Sevenday from "/src/assets/images/download (2).png"
import BestCustomer from "/src/assets/images/download (3).png"


const Services = () => {
  
    const polices =[
        {
          img:"data:image/png;base64,",
          title:"Easy Exchange Policy",
          description:"We Offers hassles free exchange Policy",
        },
        {
          img:"data:image/png;base64,",
          title:"7 Days Return Policy",
          description:"We Provide 7 days free return policy",
        },
        {
          img:"data:image/png;base64,",
          title:"Best Customer Support",
          description:"We Provide 24/7 Customer Support",
        },



    ];
     
  return (
    <>
    <div className="sevices py-32  ">

        <div className="">
        <ul className='lg:flex lg:justify-around  ' >

        {
            polices.map((data,index)=>(
             <li key={index} className='py-7' > 
             <img src={data.img} alt="polices img" className='mx-32 ' />
             <h1 className='text-2xl font-semibold text-center ' >{data.title}</h1>
             <p className='text-xl text-gray-600 text-center ' >{data.description}</p>
             
             </li>
                
            ))
        }
        </ul>
        
               </div>








        {/* <div className="easy-exhange">
         <img src={Easyexchange} alt="Easyexchange" className='' />
         <h1 className="text-xl text-center" >Easy Exchange Policy</h1>
        </div>
        <div className="7-Days-Return"></div>
        <div className="Best-Customer-Support"></div>
         */}
    </div>
    </>
  )
}

export default Services